...
void wait(float val){
	uint32_t milisecond =(uint32_t) val * 1000;
...
void writeByte(uint8_t address, uint8_t subAddress, uint8_t data) {
	...
	HAL_I2C_Master_Transmit(&hi2c1, (uint16_t)address, data_write, 2, 100);
...
char readByte(uint8_t address, uint8_t subAddress) {
	...
	HAL_I2C_Master_Transmit(&hi2c1, (uint16_t)address, data_write, 1, 100);
	HAL_I2C_Master_Receive(&hi2c1, (uint16_t)address, data, 1, 100);
...
void readBytes(uint8_t address, uint8_t subAddress, uint8_t count,
	...
	HAL_I2C_Master_Transmit(&hi2c1, (uint16_t)address, data_write, 1, 100);
	HAL_I2C_Master_Receive(&hi2c1, (uint16_t)address, data, count, 100);
	//i2c.write(address, data_write, 1, 1); // no stop
...
uint8_t readMpuName() {
	uint8_t name = readByte(MPU9250_ADDRESS, WHO_AM_I_MPU9250);
	return name;
}
...
// can join the I2C bus and all can be controlled by the Arduino as master
	...
	writeByte(MPU9250_ADDRESS, INT_PIN_CFG, 0xb2);
